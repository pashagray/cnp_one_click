...
module CnpOneClick
  class CardRegistrationComplete

    PERMITTED_PARAMS = %i(merchant_id user_id card_id test_amount wsdl endpoint)
    REQUIRED_PARAMS  = %i(merchant_id user_id card_id test_amount wsdl endpoint)
    ...
    def request!
      ...
      request = client.call(:complete_card_registration, message: hash_to_send )
      response(request.body[:complete_card_registration_response][:return])
    end
